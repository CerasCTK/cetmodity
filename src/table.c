...
#define PRINT_BORDER_X(border_len) (printf("%.*s\n", border_len, TABLE_BORDER_X))
...
void show_order_table(const dllist_order list_order) {
    ...
    const int border_x = INDEX_LEN + UUID_STR_LEN + SENDER_MAX_NAME_LEN + RECEIVER_MAX_NAME_LEN + PRODUCT_MAX_NAME_LEN + NUM_OF_SEPARATE_CHAR + QUANTITY_ITEMS;
    ...
    for (int i = 0; i < do_size(list_order); i++) {
        ...
        for (int j = 1; j < item_list_size; j++) {
            ...
            printf("%*s|\n", PRODUCT_MAX_NAME_LEN + QUANTITY_ITEMS, item_info_array[j]);
        }
